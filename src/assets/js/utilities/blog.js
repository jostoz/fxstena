...
class Blog {
    ...
    contentTruncate() {
        this.articleWrap.forEach(txt => {
            ...
            let truncateText = this.trimLongTitle(currentText.innerText, 170)
    ...
    createCategoryWidget(data) {
        if(document.querySelector('.widget-categories') !== null) {
            ...
            categoryFilter.forEach(eachData => {
                this.categoryWrap.innerHTML += `<li><a href="${this.sanitizePath}-find.html?category=${eachData.category.toLowerCase()}" class="link-dark text-decoration-none d-flex justify-content-between align-items-center">${eachData.category}<span class="badge bg-primary text-white rounded-1">${eachData.totalPost}</span></a></li>`
            })
        }
    }
    ...
    searchProcess(data) {
        ...
        if(document.querySelector('[data-title="blog-find"]') && params.has('result')) {
            ...
            if(this.checkDiff(notFoundArr) !== true) {
                ...
                notFoundDiv.innerHTML = `<article class="card shadow-sm mb-4 mb-lg-0">
    ...
    postFormat({link, title, content, author, date, category}) {
        return `<article class="card shadow-sm mb-4">
    ...
    <div class="card-footer blog-footer d-flex justify-content-between align-items-center px-3 px-md-4 py-2">
        ...
        <a href="${this.sanitizePath}/${link}" class="btn btn-link text-decoration-none p-0">Read more<i class="fas fa-circle-chevron-right ms-1"></i></a>
    </div>
</article>`
    }
